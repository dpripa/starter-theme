...
final class Str {
	public static function to_camelcase(string $text, string $separator = '_'): string {
		return array_reduce(
			...
			function (string $result, string $item): string {
				return $result ? $result . ucfirst($item) : $item;
			},
			''
		);
	}

	public static function generate_random_str(int $length = 16, string $keyspace = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'): string {
		$pieces = [];
	...
	public static function truncate(string $string, int $length = 100, array $args = []): string {
		$args = wp_parse_args(
			$args,
			[
				'ending' => '...',
