<?php

namespace MyTheme;

defined( 'ABSPATH' ) || exit;
?>
<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
	<?php if ( has_post_thumbnail() ) { ?>
		<a class="mnt-img" href="<?php echo esc_url( get_permalink() ); ?>">
			<img
				data-src="<?php the_post_thumbnail_url( 'ego_card' ); ?>"
				data-retina="<?php the_post_thumbnail_url( 'ego_card_retina' ); ?>"
				alt="<?php echo esc_attr( __( 'Thumbnail for', 'my-theme' ) . ': "' . get_the_title() . '"' ); ?>"
			>
		</a>
		<?php
	}

	the_title(
		'<h3><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">',
		'</a></h3>'
	);
	?>
	<div>
		<?php the_excerpt(); ?>
	</div>
	<?php if ( has_tag() ) { ?>
		<div><?php the_tags( '', '' ); ?></div>
	<?php } ?>
	<div>
		<ul>
			<li><?php the_category( ', ' ); ?></li>
			<li><?php echo get_the_date(); ?></li>
		</ul>
	</div>
	<a
		href="<?php echo esc_url( get_permalink() ); ?>"
		aria-label="<?php echo esc_attr( __( 'Read more', 'my-theme' ) . ': "' . get_the_title() . '"' ); ?>"
	>
		<?php echo esc_html( __( 'Read more', 'my-theme' ) ) . ' →'; ?>
	</a>
</article>
